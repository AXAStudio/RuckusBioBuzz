 ...
 * system is corrected.
 *
 * <p>RUCKUS PATCH: upstream deleted this class (and the whole {@code control} package) in v3. It is
 * retained, with the 2.1.2 fork's integral patches, because every swerve turn gain on this robot was
 * measured against exactly this arithmetic and v3's {@code PIDController} differs from it (no
 * integral clamp, band or reset threshold; a 1 ms dt floor that drops the D term). It implements
 * v3's {@link Controller} so it plugs into {@code CoaxialPodConfig.turnController} unchanged - see
 * {@link #calculate(double, double)}.
 *
...
public class PIDFController implements Controller {
    ...
    /**
     * RUCKUS PATCH: v3 {@link Controller} adapter. {@code target} is taken as the feed-forward
     * input - which is how v3's CoaxialPod uses it (it passes the turn direction, or 0 inside 2
     * degrees), and exactly what the 2.1.2 pod fed {@link #updateFeedForwardInput}. The result is
     * the same arithmetic as {@code updateFeedForwardInput(target); updateError(error); run()}.
     */
    @Override
    public double calculate(double target, double error) {
        updateFeedForwardInput(target);
        updateError(error);
        return run();
    }

    /**
     * RUCKUS PATCH: v3 {@link Controller} adapter, derivative-on-measurement form. Follows v3's
     * convention that the third argument is the measured velocity, so d(error)/dt is its negation
     * - see {@link #updateErrorWithDerivative}.
     */
    @Override
    public double calculate(double target, double error, double measuredVelocity) {
        updateFeedForwardInput(target);
        updateErrorWithDerivative(error, -measuredVelocity);
        return run();
    }
