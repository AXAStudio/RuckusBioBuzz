...
- `Settings → Field Map` offers **BIOBUZZ Field (2026-2027)**, the default-list entry above DECODE. `public/fields/biobuzz.webp` is generated, not downloaded: `scripts/gen_biobuzz_field.py` draws it at 4096² from the BIOBUZZ Competition Manual V1 — quoted dimensions where the manual gives them (FLOWER, HIVE frame and CELL, LOADING ZONE, GARDEN, SCORING ELEMENT sizes, Fig 10-2 staging) and measurements off its Figure 9-2 top view, anchored to the TILE seams, where it does not. The script header lists which is which. Orientation follows the manual: audience at the bottom of the image, red ALLIANCE on the left, so in visualizer coordinates x = 0 is the red wall and y = 0 the audience wall. Palette and line weights are sampled from `decode.webp` so the four field maps read as one set. The default field map is unchanged (DECODE).
