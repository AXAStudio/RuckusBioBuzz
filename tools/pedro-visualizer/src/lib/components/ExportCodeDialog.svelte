<script lang="ts">
  ...
  let teamCodeClassName = "GeneratedSwerveAuto";
  let exportedCode = "";
    ...
    >
      <div class="flex flex-row justify-between items-center w-full">
        <p class="text-sm font-light text-neutral-700 dark:text-neutral-400">
          ...
          {:else if exportFormat === "teamcode"}
            Here is the TeamCode PathStep autonomous for this path:
          {/if}
        </p>
        <div class="flex items-center gap-2">
          ...
          {:else if exportFormat === "teamcode"}
            <div class="flex items-center gap-2">
              <label
                for="teamcode-class-name"
                class="text-sm font-light text-neutral-700 dark:text-neutral-400"
                >Class Name:</label
              >
              <input
                id="teamcode-class-name"
                type="text"
                bind:value={teamCodeClassName}
                on:input={refreshTeamCodeAutoCode}
                class="px-2 py-1 text-sm rounded-md bg-neutral-100 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500 w-44"
                placeholder="GeneratedSwerveAuto"
              />
            </div>
          {/if}
